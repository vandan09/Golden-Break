...
- [ ] Screen 4 — Game Over: compact GAME OVER label, large score, "New best!" pill badge, inline ceramic progress, gold-glow Continue button with an "AD" tag, outline-style Play Again, small "2x Double coins" text link. Can reuse `UiCeramicPreview` (new, built for Home's card) directly for the inline ceramic progress.
- [ ] Screen 5 — Gallery: 2-column cards with the *actual* per-tier ceramic silhouette (bowl/vase/plate — real distinct shapes, sourced from the design's SVG path data), not a flat colour-square thumbnail. **Silhouette + crack rendering itself is now done** (see below) — this task is really just GalleryScreen's card layout catching up to use `CeramicSilhouetteSprite`/`UiCeramicPreview` instead of its current flat-square thumbnail.
...
**Project-wide follow-up this Home screen rework surfaced:** `HomeScreen`'s Canvas is now the only one using `CanvasScaler.ScaleMode.ScaleWithScreenSize` against the mockup's 390×844 reference resolution — every other screen (`GameplayHUD`, `GalleryScreen`, `SettingsScreen`, `GameOverScreen`, etc.) still uses raw 1:1 pixel `CanvasScaler` defaults, so mockup pixel values copied there directly would render far too small on real devices (confirmed: GameplayHUD's existing 160×56 buttons are already proportionally smaller on a 1080-wide test device than in the 390-wide mockup). Apply the same `ScaleWithScreenSize`/390×844 setup to each canvas as it gets its own Claude Design pass, so the whole app scales consistently once all 6 screens are done — a temporarily inconsistent transition period between old and new screens is expected and fine given the player's own "one screen at a time" sequencing.
