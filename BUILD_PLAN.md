...
- [x] Implement palette system as ScriptableObjects (Golden Dark default + 1 additional) — `PaletteDefinition` (new) + `UiPalette.Active`/`SetActive`; every existing call site (`UiPalette.Background`, `GetBlockColour`, etc.) reads from the active palette unchanged. Second palette is "Jade Dusk," a cool reskin of the UI chrome only — gameplay block colours deliberately stay identical across themes (legibility, not cosmetics). No UI to switch palettes yet (that's the theme-pack IAP purchase flow / Claude Design work, not this task)
...
- [x] Screen 1 — Home: ceramic-preview card above Play, gold-bordered glowing Play button with play-triangle icon, Daily Challenge button with streak-fire badge, icon-based Gallery/Settings buttons. Rebuilt in full against the mockup's exact 390×844 pixel values (Canvas now uses `ScaleWithScreenSize` against that reference resolution, a new project-wide need — see note below). Verified via a real rendered screenshot (Editor Play Mode, `Assets/Tests/PlayMode/HomeScreenSmokeTests.cs`, camera-captured since ScreenSpaceOverlay can't be read back in `-batchmode`), not just "it compiles." Known simplifications, all deliberate and documented inline in code: solid background instead of the mockup's barely-visible 2-stop gradient; Gallery icon is outline-only on both overlapping squares (the mockup's exact icon relies on one square's fill matching one specific parent background colour to fake a cutout, which doesn't generalize to this icon's different real parents); streak badge shows a procedurally-drawn flame glyph instead of the mockup's 🔥 emoji character (Unity's legacy uGUI `Text` has no colour-emoji glyph support at all — confirmed by testing, not assumed).
- [ ] Screen 2 — Gameplay: rounded (9px) grid cells with inset highlight/shadow, ceramic + gradient progress bar above the board, small circular icon buttons for undo/refresh (not bottom text buttons), rounded tray cards showing each piece as a mini-grid preview. **Caution for whoever picks this up: the open grid-cell rendering bug (PROGRESS.md's "OPEN BUG" section) touches exactly `GridManager`'s cell `SpriteRenderer`s — reproduce/rule out that bug BEFORE reworking this screen's cell rendering, so it isn't debugged twice or accidentally masked by unrelated visual changes.**
...
- [x] Screen 4 — Game Over: compact GAME OVER label, large score, "New best!" pill badge, inline ceramic progress, gold-glow Continue button with an "AD" tag, outline-style Play Again, small "2x Double coins" text link. Rebuilt in full against the mockup's exact pixel values, reusing `UiCeramicPreview` (built for Home's card) for the inline ceramic progress. Milestone banner (existing functionality, not in the mockup) routed through the existing toast system instead of silently dropped. Verified via a real rendered screenshot (`ScreenVerificationSmokeTests`) — caught and fixed a real bug along the way: the "GAME OVER" letter-spacing hack (inserting plain ASCII spaces between characters, since legacy uGUI `Text` has no real tracking property) rendered far too wide; switched to a thin space (U+2009).
- [x] Screen 5 — Gallery: 2-column cards with the *actual* per-tier ceramic silhouette (bowl/vase/plate — real distinct shapes, sourced from the design's SVG path data), not a flat colour-square thumbnail. Rebuilt the card grid using `UiCeramicPreview`/`CeramicSilhouetteSprite` (fully gold cracks, since a gallery entry is always a completed ceramic) plus a back-chevron header. Verified via a real rendered screenshot with seeded test data (not just the empty state) — caught and fixed two real layout bugs: `RefreshCards()` was computing the 2-column cell width from a live `RectTransform.rect.width` read *before* the panel was even active (uGUI hadn't resolved a real size yet, producing one wildly oversized card), and the grid content's `sizeDelta` was left at its non-zero default, which for a horizontally-stretched anchor acts as an *additive* width offset, silently overflowing the content area (all-cards-shifted-left-with-text-clipped bug). Fixed both; a rendered screenshot now shows a correctly symmetric, fully-legible 2-column grid.
- [x] Screen 6 — App icon (512×512): kintsugi crack motif on a dark circular badge, per the mockup's exact SVG. `AppIconGenerator` (new editor tool) rasterizes the mockup's exact SVG coordinates (radial-gradient badge, inner circle, 3 glowing gold crack lines with a highlight overlay) to a real `Assets/Icons/AppIcon.png` and assigns it to every Android `PlayerSettings` icon slot. Visually verified by reading the generated PNG directly — matches the mockup closely.
- [x] Real bowl/vase/plate silhouette + crack-path data translated to Unity, replacing the flat-rectangle placeholder for every tier — done differently than originally scoped (not `LineRenderer`/mesh, see below), but the same real result: `CeramicSilhouetteSprite` (new) rasterizes the mockup's exact SVG ellipse/bezier-path coordinates for all 3 shapes the design actually supplies (bowl/vase/plate) into cached sprites; `CeramicAssetGenerator` (editor tool) now assigns each of CLAUDE.md's 9 tiers the closest of those 3 shapes by its own display name (round vessels → bowl, "plate"/"ornate plate" → plate, "tall vase" → vase — nothing supplies 9 distinct shapes) and generates cracks fanning downward from the rim (matching the mockup's own crack style) instead of the old full-360°-starburst placeholder, scaled per shape's own extent. `CeramicView` (world-space, main gameplay ceramic) and the new `UiCeramicPreview` (plain uGUI, for compact previews like Home's card) both consume this — one shared implementation, not two.
...
**Project-wide follow-up this Home screen rework surfaced, now shared infrastructure:** `ResponsiveCanvasSetup` (new) centralizes `CanvasScaler.ScaleMode.ScaleWithScreenSize` against the mockup's 390×844 reference resolution, plus a `Screen.safeArea`-respecting wrapper every screen's real content parents under (production-quality requirement: "no text should hide under phone screen" — notches/punch-hole cameras/rounded corners). `HomeScreen`, `GameOverScreen`, and `GalleryScreen` all use it now. `GameplayHUD`, `SettingsScreen`, and `DailyChallengeUI` still use raw 1:1 pixel `CanvasScaler` defaults and no safe-area handling — apply `ResponsiveCanvasSetup` to each as it gets its own Claude Design pass (GameplayHUD is Screen 2, still on hold; Settings/DailyChallengeUI aren't in the 6-screen mockup at all, so use the same helper directly rather than inventing a different pattern). A temporarily inconsistent transition period between old and new screens is expected and fine given the player's own "one screen at a time" sequencing.
